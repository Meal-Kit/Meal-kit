...
public class MaterialService {
    ...
    public void applyMaterial(Kit kit, String materialContents) {
        ...
        List<String> keywordContents =
                Arrays.stream(materialContents.split(","))
                        .map(String::trim)
                        .filter(s -> s.length() > 0)
        ...
        for (Material oldMaterial : oldMaterials) {
            boolean contains =
                    keywordContents.stream()
                            .anyMatch(s -> s.equals(oldMaterial.getMaterialKeyword().getContent()));
        ...
        needToDelete.forEach(
                hashTag -> {
                    materialRepository.delete(hashTag);
                });

        keywordContents.forEach(
                keywordContent -> {
    ...
    private Material saveMaterial(Kit kit, String keywordContent) {
        ...
        Optional<Material> opMaterial =
                materialRepository.findByKitIdAndMaterialKeywordId(kit.getId(), keyword.getId());

        if (opMaterial.isPresent()) {
            return opMaterial.get();
        }

        Material material = Material.builder().kit(kit).materialKeyword(keyword).build();
